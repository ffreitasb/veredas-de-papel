...
class StatisticalThresholdsConfig(BaseSettings):
    ...
    changepoint_min_size: int = Field(default=5, description="Tamanho minimo do segmento")

    # Rolling Z-Score
    rolling_window: int = Field(default=14, description="Janela em dias")
    rolling_z_medium: Decimal = Field(default=Decimal("2.5"), description="Z-score > X = MEDIUM")
    rolling_z_high: Decimal = Field(default=Decimal("3.5"), description="Z-score > X = HIGH")
...
class MLThresholdsConfig(BaseSettings):
    ...
    # Isolation Forest
    if_contamination: float = Field(default=0.05, description="Percentual esperado de anomalias")
    ...
    if_score_threshold_medium: float = Field(default=-0.3, description="Score < X = MEDIUM")
    if_score_threshold_high: float = Field(default=-0.5, description="Score < X = HIGH")

    # DBSCAN
    dbscan_eps: float = Field(default=0.5, description="Raio do cluster")
    dbscan_min_samples: int = Field(default=3, description="Minimo de amostras por cluster")
    dbscan_score_threshold_medium: float = Field(default=2.0, description="Distancia > X = MEDIUM")
    dbscan_score_threshold_high: float = Field(default=3.0, description="Distancia > X = HIGH")
...
class Settings(BaseSettings):
    ...
    statistical: StatisticalThresholdsConfig = Field(default_factory=StatisticalThresholdsConfig)
