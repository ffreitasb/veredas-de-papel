"""
Coletor de CDBs da Rico Investimentos.

Estratégia: Approach A — scraping HTML via Playwright (JS-rendered).
# TODO: migrar para API não-oficial (Approach B)

A Rico opera sobre infraestrutura XP; os seletores tendem a ser
similares aos da XP mas em domínio diferente.
...
# Rico usa infraestrutura XP — seletores provavelmente similares
...
_URL = "https://www.rico.com.vc/renda-fixa"
_FONTE = "rico"


class RicoCollector(WebCollectorBase):
    """Coleta prateleira de CDB/RF da Rico Investimentos."""
    ...
    async def collect(self) -> CollectionResult:
        ...
        try:
            ...
            logger.error("Rico: falha ao carregar página — %s", exc)
            return CollectionResult.error(str(exc), source=self.SOURCE)

        ofertas = self._parse(html)
        logger.info("Rico: %d ofertas extraídas", len(ofertas))
    ...
    def _parse(self, html: str) -> list[CDBOferta]:
        ...
        if not cards:
            logger.warning(
                "Rico: nenhum card encontrado — seletor '%s' pode estar desatualizado",
                _CARD_SEL,
            )
            return []
    ...
    def _parse_card(self, card) -> CDBOferta | None:
        try:
            ...
            logger.debug("Rico: erro ao parsear card — %s", exc)
            return None
