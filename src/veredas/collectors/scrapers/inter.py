...
_WAIT_FOR = "[class*='InvestmentCard'], [class*='product-card'], .investment-item, .renda-fixa-card"
_CARD_SEL = "[class*='InvestmentCard'], [class*='investment-card'], .product-item"
_EMISSOR_SEL = "[class*='issuer'], [class*='emissor'], .bank-name, .emitter-name"
_TAXA_SEL = "[class*='rate'], [class*='taxa'], [class*='yield'], .investment-rate"
_PRAZO_SEL = "[class*='term'], [class*='prazo'], [class*='maturity'], .investment-term"
_VALOR_MIN_SEL = "[class*='minValue'], [class*='valorMinimo'], [class*='minimum'], .min-investment"
_LIQUIDEZ_SEL = "[class*='liquidity'], [class*='liquidez'], .liquidity-badge"

_URL = "https://www.bancointer.com.br/investimentos/renda-fixa/"
_FONTE = "inter"


class InterCollector(WebCollectorBase):
    """Coleta prateleira de CDB/RF do Banco Inter."""
    ...
    async def collect(self) -> CollectionResult:
        ...
        try:
            ...
            logger.error("Inter: falha ao carregar página — %s", exc)
            return CollectionResult.error(str(exc), source=self.SOURCE)

        ofertas = self._parse(html)
        logger.info("Inter: %d ofertas extraídas", len(ofertas))
    ...
    def _parse(self, html: str) -> list[CDBOferta]:
        ...
        if not cards:
            logger.warning(
                "Inter: nenhum card encontrado — seletor '%s' pode estar desatualizado",
                _CARD_SEL,
            )
            return []
    ...
    def _parse_card(self, card) -> CDBOferta | None:
        try:
            ...
            logger.debug("Inter: erro ao parsear card — %s", exc)
            return None
